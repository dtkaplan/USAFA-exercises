...
```{r echo=FALSE}
knitr::include_graphics("/images/aarp-rates.png")
```
...
# Exercise 24: The parameter k in exponentials

Each of the following reflects a function of the form $f(t) = A \exp(kt) + B$ versus $t$.  Provide a reasonable numerical value for $k$ good to about 10%.  (Hint: In terms of the doubling time $\tau$, remember that $k = \ln(2)/\tau$.)  Your answer should always be in the form of $k$, but you are welcome to provide intermediate calculations or annotations on the graph to support your claim.

a. 
```{r one,echo=FALSE}
plotFun( A*exp(k*t) + B~t, tlim=range(0,10), A=10, B=0,k=-.5, ylab="",main="Graph (a)")
```
 

#. 
```{r twob,echo=FALSE}
plotFun( A*exp(k*t) + B~t, tlim=range(0,10), A=-10,B=10,k=-.25,ylab="",main="Graph (b)")
```

#. 
```{r threeb,echo=FALSE}
plotFun( Re(A*exp(k*t)) ~t, tlim=range(0,10), A=10,k=-.25 + 1i*2*pi/2,ylab="",main="Graph (c)")
```
 
#. The value of and initial deposit of $100 as it compounds at an  interest  rate of 3% per year.
  
#. The amount of the radioactive isotope P$_{32}$ over time.  After 150 days, only about 1/1000 an initial amount $A$ will remain.


# Exercise 25: Evaporating alcohol?
...
For each of the following functional formulas, use `fitModel()` to find the appropriate parametersand plot the resulting model function along with the data.
...
Fluctuating humidity in museum exhibition halls is a serious problem because it leads to degradation of many valuable and irreplaceable artifacts.  The goal is to install a climate control system that maintains constant humidity.   The figure displays humidity records for two years at one of the world's great natural history museums.  Each  black rectangle shows fluctuation during a week.

```{r echo = FALSE, fig.cap="Relative humidity for two years in the Field Museum in Chicago. [Source](http://cool.conservation-us.org/jaic/articles/jaic30-02-006.html)"}
knitr::include_graphics("/images/humidityfluctuations.png")
```
...
2. Assuming an {\bf exponential model}, what will be the air pressure at 30,000 feet?
...
3. Which of the two models seems more reasonable?  Say why.
\answerSpace{.5in}

<!--begin-answer-->
The linear model shows the pressure becoming negative at 30,000 feet. 
We know that pressure can't become negative.  As altitude goes up, air pressure decreases toward zero.  This suggests an exponential model. 
<!--end-answer-->


*Not part of the problem*: Just for interest, note that the numbers have been rounded a bit.  The actual pressure at sea level is 1.03 kg per square cm, and at 5000 ft altitude it's 0.86 kg per square cm.  But use the round numbers in your calculations. Denver is really at an altitude of about 5200 feet above sea level.)
...
The figures below all show the same relationship $f(x)$, drawn as a thick, gray curve.  In addition, each figure shows a modeling approximation -- g1, g2, g3, or g4 -- to $f(x)$.  Your task is to decide how good an approximation each one is, according to several different criteria.

Your answers will be chosen from among:

* Good.  Overall, the approximation matches to better than, say, 5%.
...
#. Approximation $g2$
    ...
    ```{r echo=FALSE}
graphFun(f(x) ~ x, xlim=c(0,100), color="gray", lwd=3) %>%
  graphFun(g2(x) ~ x, color = "blue" ) %>%
  gf_labs(title = "Approximation g2")
```
