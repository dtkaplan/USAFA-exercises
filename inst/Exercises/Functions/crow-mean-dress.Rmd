...
`r etude::exercise_title()` The surface area $S$ of a mammal is reasonably well approximated by the function $S(M) \equiv k (M/1 kg)^{2/3}$ where $M$ is the body mass and the constant $k$ depends on the particular species under consideration. (The point of dividing M by "1 kg" is to render dimensionless the quantity  M/1kg, as required when applying  a non-integer power.) 
